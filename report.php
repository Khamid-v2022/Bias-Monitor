...
<?php 
  $query_score_histogram = "SELECT detail.date_of_article, IFNULL(SUM(bias1_neg), 0) bias1_neg, IFNULL(SUM(bias1_neu), 0) bias1_neu, IFNULL(SUM(bias1_pos), 0) bias1_pos, IFNULL(SUM(bias2_neg), 0) bias2_neg, IFNULL(SUM(bias2_neu), 0) bias2_neu, IFNULL(SUM(bias2_pos), 0) bias2_pos 
  FROM (
    SELECT * FROM bias_detail detail 
    WHERE bias_id = " . $_GET['id'] . ") detail
  LEFT JOIN ( 
    SELECT a.*, IFNULL(b.bias2_neg, 0) bias2_neg, IFNULL(b.bias2_neu, 0) bias2_neu, IFNULL(b.bias2_pos, 0) bias2_pos 
    FROM ( 
      SELECT bias_detail_id, SUM(is_neg) bias1_neg, SUM(is_neu) bias1_neu, SUM(is_pos) bias1_pos 
      FROM article_sentences 
      WHERE bias_selected = 'bias_1' AND bias_master_id = " . $_GET['id'] . " 
      GROUP BY bias_detail_id) a 
    LEFT JOIN ( 
      SELECT bias_detail_id, SUM(is_neg) bias2_neg, SUM(is_neu) bias2_neu, SUM(is_pos) bias2_pos 
      FROM article_sentences 
      WHERE bias_selected = 'bias_2' AND bias_master_id = " . $_GET['id'] . " 
      GROUP BY bias_detail_id) b 
    ON a.bias_detail_id = b.bias_detail_id 

    UNION 

    SELECT b.bias_detail_id, IFNULL(bias1_neg, 0) bias1_neg, IFNULL(bias1_neu, 0) bias1_neu, IFNULL(bias1_pos, 0) bias1_pos, b.bias2_neg, b.bias2_neu, b.bias2_pos 
    FROM ( 
      SELECT bias_detail_id, SUM(is_neg) bias1_neg, SUM(is_neu) bias1_neu, SUM(is_pos) bias1_pos 
      FROM article_sentences 
      WHERE bias_selected = 'bias_1' AND bias_master_id = " . $_GET['id'] . " 
    ...
    RIGHT JOIN ( 
      ...
      WHERE bias_selected = 'bias_2' AND bias_master_id = " . $_GET['id'] . " 
      GROUP BY bias_detail_id) b 
    ON a.bias_detail_id = b.bias_detail_id ) sentences 
  ...
  if($results->num_rows > 0){
      while($item = $results->fetch_assoc()) {
          ...
          array_push($histogram_bias1_neg, $item['bias1_neg']); 
          array_push($histogram_bias1_neu, $item['bias1_neu']); 
          array_push($histogram_bias1_pos, $item['bias1_pos']);  
          if($exist_topic2){
    ...
    <div class="container">
      <div class="row">
        ...
        <div style="display: flex; align-items: center; justify-content: end;">
          ...
          <select class="form-control" id="sort_select" style="width: 150px">
            <option value="date_of_article" <?=$order_by=="date_of_article"?"selected":""?>>Date</option>
            <option value="bias1_neg" <?=$order_by=="bias1_neg"?"selected":""?>><?=$topics[0]?> Score</option>
            <?php 
            if($exist_topic2){ ?>
                <option value="bias2_neg" <?=$order_by=="bias2_neg"?"selected":""?>><?=$topics[1]?> Score</option>
            <?php }
            ?>
        ...
        <div class="table-responsive" style="padding:15px;"> 
           <table id="bias_master_data" class="table table-striped table-bordered">  
              ...
              <?php  
                  $query4 = "SELECT detail.*, sentences.*, IFNULL(c.number_of_count, 0) number_of_count FROM (
                  ...
                  LEFT JOIN (
                    ...
                    SELECT b.bias_detail_id, IFNULL(bias1_neg, 0) bias1_neg, IFNULL(bias1_neu, 0) bias1_neu, IFNULL(bias1_pos, 0) bias1_pos, b.bias2_neg, b.bias2_neu, b.bias2_pos 
                    FROM ( 
                      SELECT bias_detail_id, SUM(is_neg) bias1_neg, SUM(is_neu) bias1_neu, SUM(is_pos) bias1_pos FROM article_sentences 
                      WHERE bias_selected = 'bias_1' AND bias_master_id = " . $_GET['id'] . " 
                      GROUP BY bias_detail_id) a 
                    RIGHT JOIN ( 
                      SELECT bias_detail_id, SUM(is_neg) bias2_neg, SUM(is_neu) bias2_neu, SUM(is_pos) bias2_pos FROM article_sentences 
                      WHERE bias_selected = 'bias_2' AND bias_master_id = " . $_GET['id'] . " 
                      GROUP BY bias_detail_id) b 
                    ON a.bias_detail_id = b.bias_detail_id ) sentences 
                  ON detail.id = sentences.bias_detail_id 
                  LEFT JOIN (
                    SELECT bias_detail_id, COUNT(id) AS number_of_count 
                    FROM article_sentences 
                    WHERE bias_master_id = " . $_GET['id'] . " GROUP BY bias_detail_id) c
                  ON detail.id = c.bias_detail_id
                    ...
                    while($row = $results->fetch_assoc()) {
                      echo '<tr>'; 
                        ...
                        if($exist_topic2){
                          ...
                          echo '<td>' . $topics[0] . ": " . ($row['bias1_neg']?$row['bias1_neg']:0) . '<br/>';
